...
export default function DashboardPodcastPage({ podcasts }: any) {
  ...
  const getUploadStatus = (podcast: any) => {
    if (!podcast.pdc_jadwal_upload && podcast.pdc_link) {
      return { text: "Sudah Diupload", color: "green" };
    }
    if (!podcast.pdc_jadwal_upload) {
      return { text: "Tidak Dijadwalkan", color: "red" };
    }
    if (!podcast.pdc_link) {
      return { text: "Belum Diupload", color: "red" };
    }
    return { text: "Sudah Diupload", color: "green" };
  };
      ...
      <CardContent className="space-y-2">
        {podcasts.map((podcast: any, index: any) => (
          <Link href={"/podcast"} key={podcast.pdc_id}>
            <div className="mb-2 bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100">
